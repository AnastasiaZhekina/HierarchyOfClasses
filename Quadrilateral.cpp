...
Quadrilateral::Quadrilateral(string p_name, int p_a, int p_b, int p_c, int p_d, int p_A, int p_B, int p_C, int p_D) {
	if (360 == p_A + p_B + p_C + p_D) {
		a = p_a; b = p_b; c = p_c; d = p_d;
		A = p_A; B = p_B; C = p_C; D = p_D;
		name = p_name;
	}
	else throw domain_error("Сумма сторон должна быть ровна 360");
	}
	...
	void Quadrilateral::print_info(){
		cout << gets() << "(стороны " << getxa() << ", " << getxb() << ", " << getxc() << ", " << getxd() << "; ";
		cout << "углы " << getxA() << ", " << getxB() << ", " << getxC() << ", " << getxD() << ") " << "создан " << endl << endl;
	}
